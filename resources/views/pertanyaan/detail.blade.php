    ...
    <div class="card card-widget">
        <div class="card-header">
            <div class="user-block">
                ...
                <span class="description text-bold">
                    <h3> <a href="/pertanyaan/{{ $question->id }}/{{ $question->slug }}">{{ $question->judul }}</a></h3>
                </span>

                <span class="description text-sm">
                    <span class="badge bg-primar link-black text-sm"><small><i class="far fa-thumbs-up"></i> Like (<i id="vote-count">{{ $question->likedislikes->sum('value') }}</i>)</small></span>
        ...
        <div class="card-body">
            ...
            <div class="btn-group float-right topic ml-1" data-post="{{ $question->id }}">
                <button type="button" class="btn btn-outline-primary btn-sm vote-pertanyaan {{ $question->likedislikes && $question->likedislikes->contains('user_id', Auth::id()) ? ($question->likedislikes->where('user_id', Auth::id())->first()->value > 0 ? 'active' : null) : null}}" data-value="1" data-pertanyaan-id="{{ $question->id }}"><i class="far fa-thumbs-up mr-1"></i></button>
                <button type="button" class="btn btn-outline-primary btn-sm vote-pertanyaan {{ $question->likedislikes && $question->likedislikes->contains('user_id', Auth::id()) ? ($question->likedislikes->where('user_id', Auth::id())->first()->value < 0 ? 'active' : null) : null}}" data-value="-1" data-pertanyaan-id="{{ $question->id }}"><i class="far fa-thumbs-down mr-1"></i></button>
            </div>
            @auth
...
<section class="content">
    <div class="card card-widget">
        <div class="card-header">
            <h5 class="header-title">Jawaban ( <span class="color">{{ $question->jawaban->count() }}</span> )</h5>
        </div>
        <div class="card-body">
